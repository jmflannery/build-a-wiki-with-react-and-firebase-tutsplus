...
var firebase = new Firebase('https://sweltering-torch-2142.firebaseio.com/');
...
router.use(require('express-session')({
  ...
  secret: 'asldjfa9osd8fh34ouifne8f3672gi3y2b7t23g8723biuydguyfga'
}));
...
router.post('/api/signup', function(req, res) {
  ...
  users.child(username).once('value', function(snapshot) {
    if (snapshot.exists()) {
      return res.json({ signedIn: false, message: 'username is taken' });
    }
    var userObj = {
      username: username,
